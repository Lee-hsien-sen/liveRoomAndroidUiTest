...
class Student(object):
    def __init__(self, live_id, user_id, nick_name, user_type, room_index,msn_ip):
        self.liveId = live_id
        self.userId = user_id
    ...
    def do_join(self):
        try:
            ...
            if self.sock.connect_ex((self.msn_ip, 8114)) == 0:
                ...
                print("学生连接成功", self.sock.getpeername())
                ...
                print("学生连接失败！", self.sock.getpeername())
        except Exception as err:
            print(err)
    ...
    def do_rejoin(self):
        try:
            ...
            self.sock.settimeout(30)
            if self.sock.connect_ex((self.msn_ip, 8114)) == 0:
                print("学生重连成功", self.sock.getpeername())
                #time.sleep(1)
                self.readThread = threading.Thread(target=self.readSockData, args=(self.event,), name='Thread-read')
                self.readThread.start()
                # 重新加入房间
    ...
    def do_chat(self, content):
        try:
            ...
            if (self.isForbidden == False) and (self.chatDisabled == False) and (
                    self.groupChatDisabled == False):
                self.chatObj.content = str(content)
                chatMsg = self.chatObj.pack_stuChat(self.token, self.is_mvp, self.continue_right_num)
                self.sock.sendall(chatMsg)
                # 设置发起聊天请求时间戳
                self.chatObj.chat_req_time = int(time.time()*1000)
                # 设置响应时间间隔为0，以判断是否无响应
                self.chatObj.chat_duration = 0
            else:
                pass
        except Exception as e:
            print("网络不稳定重连中（发聊天）：", e)
    ...
    def do_vote(self, res_msg):
        if self.voteObj.voteLogic(res_msg):
            ...
            # 设置响应时间间隔为0，以判断是否无响应
            self.voteObj.submit_duration = 0
