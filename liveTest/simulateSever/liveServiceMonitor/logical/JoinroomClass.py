...
# 加入&重新加入房间
class JoinroomClass(object):
    def __init__(self, live_id, user_id, nick_name, user_type, room_index, app_id, app_secret):
        self.liveId = live_id
        self.userId = user_id
    ...
    def pack_joinReq(self):
        ...
        avatar_url_list = ["https://cdn.17xueba.com/test/server/image/2018/01/20180112111219210950.jpg",
    ...
    def joinLogic(self, resData):
        if resData.result_frame.code == 0:
            if resData.head_frame.msg_type == msg_type_pb2.JOIN_ROOM_RES:
                ...
                #
                #globalvar.set_value(self.userId,{'student_join_duration':self.join_duration})
                globalvar.update_value(self.userId, {'student_join_duration': self.join_duration})
                print("加入教室(join_room)成功！")
                print('用户:{}, 加入教室响应耗时（毫秒）: {}'.format(self.userId, self.join_duration))
                logger.info('用户:{}, 加入教室响应耗时（毫秒）: {}'.format(self.userId, self.join_duration))
                ...
                print("重新加入教室(re_join)成功！")
                print('用户:{}, 重新加入教室耗时（毫秒）: {}'.format(self.userId, self.rejoin_duration))
                logger.info('用户:{}, 重新加入教室耗时（毫秒）: {}'.format(self.userId, self.rejoin_duration))
                print("直播状态：",resData.logical_frame.re_join.live_status)
                print("#######################################################")
